# NGSS ↔ GDPT 2018 Bridge — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Viết tài liệu tiếng Việt ~18,000-22,000 từ đối chiếu **NGSS ↔ GDPT 2018** ở cấp chuẩn cho K-8, lưu tại `docs/NGSS_GDPT2018_BRIDGE.md`.

**Architecture:** 9 chương, viết tuần tự từ chương 2 (bối cảnh) → 3-5 (3 chiều NGSS) → 6-8 (phẩm chất/đánh giá/triển khai) → 9 (phụ lục) → 1 (executive summary cuối). Mỗi chương commit riêng để anh Tuấn theo dõi tiến độ trên GitHub.

**Tech Stack:** Markdown. Source gốc: `/tmp/ctkhtn.txt` (Thông tư 32/2018 KHTN 6-9, 2245 dòng). Tham khảo NGSS: kiến thức chung + `docs/PHASE_1_REPORT.md` có liên kết OpenSciEd/NGSS.

**Budget từ theo chương:** Ch 1: 500-800 | Ch 2: 1000-1400 | Ch 3: 4500-5500 | Ch 4: 3500-4500 | Ch 5: 2200-2800 | Ch 6: 1200-1500 | Ch 7: 1500-1800 | Ch 8: 1200-1500 | Ch 9: 2500-3500. Tổng: ~18,100-23,300 từ.

---

## File Structure

- **Create**: `docs/NGSS_GDPT2018_BRIDGE.md` — tài liệu chính duy nhất.
- **Create (gitignored)**: `docs/source-pdfs/gdpt-2018/yccd-parsed.json` — YCCĐ parse ra structured data để tiện tra cứu khi viết.
- **Modify**: `docs/NGSS_GDPT2018_BRIDGE_DESIGN.md` — cập nhật checkbox progress.

Không đụng các file khác. KHÔNG động đến content/, app/, components/.

---

## Task 1: Parse YCCĐ KHTN 6-9 thành structured JSON

**Files:**
- Create: `docs/source-pdfs/gdpt-2018/yccd-parsed.json` (gitignored location)
- Source: `docs/source-pdfs/gdpt-2018/ctkhtn-6-9.txt`

**Why:** Ch 3 (DCI ↔ YCCĐ) và Ch 4 (SEP) cần trích dẫn YCCĐ chính xác từ nguồn. Parse sẵn 1 lần để tránh copy sai hoặc đọc lại source nhiều lần.

- [ ] **Step 1: Đọc mục lục chi tiết để xác định boundary**

Run: `grep -n "^.\{0,10\}LỚP \|^.\{0,10\}[0-9]\+\. " docs/source-pdfs/gdpt-2018/ctkhtn-6-9.txt | head -60`
Mục tiêu: tìm line number chính xác start/end của từng lớp (6, 7, 8, 9) và từng chủ đề.

- [ ] **Step 2: Viết Python script parse YCCĐ**

Tạo file tạm `/tmp/parse_yccd.py`:
...
src = Path("docs/source-pdfs/gdpt-2018/ctkhtn-6-9.txt").read_text(encoding="utf-8")
lines = src.split("\n")

# Tìm vùng V. NỘI DUNG GIÁO DỤC → kết thúc ở VI. PHƯƠNG PHÁP
start = next(i for i, l in enumerate(lines) if l.strip().startswith("V. NỘI DUNG GIÁO DỤC"))
end = next(i for i, l in enumerate(lines) if l.strip().startswith("VI. PHƯƠNG PHÁP"))
body = lines[start:end]

# Pattern: "LỚP 6", "LỚP 7", ...
grade_re = re.compile(r"^\s*LỚP\s+(\d)\s*$")
# Pattern chủ đề: "Chủ đề N:" hoặc "1. ", "2. " ở đầu dòng ngay sau tiêu đề lớp
# Pattern YCCĐ: "–" hoặc "-" đầu dòng (bullet các yêu cầu)
...
for l in body:
    ...
    if m:
        cur_grade = m.group(1)
        grades[cur_grade] = {"topics": []}
        cur_topic = None
        continue
    if cur_grade is None:
        continue
    stripped = l.strip()
    # Topic markers thường in hoa/bold hoặc "N. Tên chủ đề"
    if re.match(r"^[A-ZĐÂÊÔƠƯ][A-ZĐÂÊÔƠƯ\s,\-]+$", stripped) and len(stripped) > 4 and len(stripped) < 80:
        cur_topic = {"name": stripped, "yccd": []}
        grades[cur_grade]["topics"].append(cur_topic)
        continue
    if cur_topic and (stripped.startswith("–") or stripped.startswith("-")):
        cur_topic["yccd"].append(stripped.lstrip("–- ").strip())

out = Path("docs/source-pdfs/gdpt-2018/yccd-parsed.json")
out.parent.mkdir(parents=True, exist_ok=True)
out.write_text(json.dumps(grades, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"Grades: {list(grades.keys())}")
for g, v in grades.items():
    print(f"  Lớp {g}: {len(v['topics'])} topics, {sum(len(t['yccd']) for t in v['topics'])} YCCĐ items")
```

Run: `cd /Users/tuanln/Documents/Claude/Projects/OpenSciedNEO && python3 /tmp/parse_yccd.py`
Expected: in ra Grades: ['6', '7', '8', '9'] kèm số topic + YCCĐ mỗi lớp.

- [ ] **Step 3: Spot-check kết quả**

Run: `python3 -c "import json; d=json.load(open('docs/source-pdfs/gdpt-2018/yccd-parsed.json')); print(json.dumps(d['6']['topics'][0], ensure_ascii=False, indent=2))"`
Expected: in ra topic đầu tiên của lớp 6 với tên rõ nghĩa + ≥3 YCCĐ items.

Nếu heuristic parsing sai (topic name không khớp, YCCĐ thiếu), điều chỉnh regex trong `/tmp/parse_yccd.py` và re-run. KHÔNG commit JSON (đã gitignored).

- [ ] **Step 4: Commit marker**

```bash
git add docs/NGSS_GDPT2018_BRIDGE_DESIGN.md  # không có thay đổi thực, bỏ qua
```
Skip commit — task này chỉ tạo file local gitignored.

---

## Task 2: Skeleton tài liệu chính + front matter + TOC

**Files:**
- Create: `docs/NGSS_GDPT2018_BRIDGE.md`

- [ ] **Step 1: Tạo file skeleton với TOC đầy đủ**

Write `docs/NGSS_GDPT2018_BRIDGE.md` với nội dung:
...
```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Init NGSS_GDPT2018_BRIDGE.md — skeleton + TOC

Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>"
git push origin main
```

Expected: push thành công, file hiện trên GitHub.

---

## Task 3: Viết Chương 2 — Bối cảnh ban hành và triết lý

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md` — thêm phần Ch 2 sau TOC

**Budget từ**: 1,000-1,400 từ tiếng Việt.

- [ ] **Step 1: Viết §2.1 NGSS (~500 từ)**

Thêm sau block "## 1. Tóm lược *(Viết sau cùng...)*" trong file:
...
- [ ] **Step 2: Viết §2.3 "So sánh triết lý" (~400-600 từ)**
...
- [ ] **Step 3: Đọc lại đoạn vừa viết, check con số và thuật ngữ**

Verify các claim cụ thể:
- NGSS công bố 4/2013 ✓ (sự thật lịch sử)
- 26 bang liên minh xây NGSS ✓
- NRC Framework 2012 ✓
- Thông tư 32/2018 ký 26/12/2018 ✓ (đã thấy trong `/tmp/ctkhtn.txt` dòng 2-8)
- 5 phẩm chất: yêu nước, nhân ái, chăm chỉ, trung thực, trách nhiệm ✓
- 3 NL chung + 7 NL đặc thù (tổng 10) ✓
- 3 thành phần NL đặc thù KHTN ✓ (đã thấy dòng 107-108 `/tmp/ctkhtn.txt`: "nhận thức khoa học tự nhiên; tìm hiểu tự nhiên; vận dụng kiến thức, kĩ năng đã học")
- 8 SEP ✓, 7 CCC ✓, 4 DCI domain (PS/LS/ESS/ETS) ✓, ~200 PE ✓
- KHTN 140 tiết/năm = 4 tiết/tuần × 35 tuần — verify:

Run: `grep -n "140 tiết\|tiết/năm\|tiết/tuần" docs/source-pdfs/gdpt-2018/ctkhtn-6-9.txt | head -10`
Expected: tìm được số tiết. Nếu sai, sửa số trong bảng.

- [ ] **Step 4: Commit chương 2**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 2 — Bối cảnh NGSS và GDPT 2018

Viết §2.1 NGSS (nguồn gốc + ba chiều), §2.2 GDPT 2018 (Thông tư
32/2018 + phẩm chất + năng lực), §2.3 so sánh triết lý.
...
## Task 4: Viết Chương 3 — DCI ↔ YCCĐ về nội dung

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 4,500-5,500 từ. Đây là chương dài nhất. Chia thành 3 sub-task step.

- [ ] **Step 1: Viết §3.1 — Cấu trúc DCI của NGSS (~1,200 từ)**

Thêm vào file sau chương 2:
...
- [ ] **Step 2: Viết §3.2 — Cấu trúc YCCĐ nội dung của GDPT 2018 (~1,200 từ)**
...
> – Nêu được khái niệm về sự nóng chảy, sự đông đặc, sự bay hơi, sự ngưng tụ, sự sôi.
...
- [ ] **Step 3: Viết §3.3 — Bảng đối chiếu domain × mạch theo grade band (~1,600 từ)**
...
- [ ] **Step 4: Viết §3.4 — Ví dụ đối sánh chi tiết (~800 từ)**
...
- [ ] **Step 5: Commit chương 3**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 3 — Đối chiếu DCI ↔ YCCĐ nội dung K-8

§3.1 DCI NGSS, §3.2 YCCĐ GDPT 2018 (TH+THCS), §3.3 bảng đối chiếu
3 grade band (K-2, 3-5, 6-8) theo 4 domain PS/LS/ESS/ETS, §3.4
ví dụ chi tiết Lớp 6 'Thể của chất'.
...
## Task 5: Viết Chương 4 — SEP ↔ Năng lực đặc thù KHTN

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 3,500-4,500 từ.

- [ ] **Step 1: Viết §4.1 — 8 SEP của NGSS (~1,000 từ)**
...
- [ ] **Step 2: Viết §4.2 — 3 thành phần năng lực đặc thù KHTN (~700 từ)**
...
- Trình bày được sự vật, hiện tượng; vai trò của các sự vật, hiện tượng và các quá trình tự nhiên bằng các hình thức biểu đạt như ngôn ngữ nói, viết, công thức, sơ đồ, biểu đồ.
...
- [ ] **Step 3: Viết §4.3 — Bảng đối chiếu 8 SEP ↔ Biểu hiện năng lực (~1,500 từ)**
...
| **2. Developing and Using Models** | Nhận thức KHTN + Tìm hiểu tự nhiên | "Trình bày được… bằng các hình thức biểu đạt như ngôn ngữ nói, viết, công thức, sơ đồ, biểu đồ" + "Sử dụng được ngôn ngữ, hình vẽ, sơ đồ, biểu bảng để biểu đạt quá trình và kết quả tìm hiểu". |
...
| **6. Constructing Explanations / Designing Solutions** | Nhận thức KHTN + Vận dụng | "Giải thích được mối quan hệ giữa các sự vật và hiện tượng" + "Nêu được các giải pháp… để bảo vệ tự nhiên". |
...
- [ ] **Step 4: Viết §4.4 — Ví dụ cụ thể từ OpenSciedNEO lesson (~800 từ)**
...
```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 4 — Đối chiếu SEP ↔ Năng lực đặc thù KHTN

§4.1 8 SEP NGSS + tiếng Việt đề xuất, §4.2 3 thành phần NL KHTN
GDPT 2018, §4.3 bảng đối chiếu 8 SEP ↔ biểu hiện NL, §4.4 ví dụ
Grade 6 Unit 1 Lesson 3 'Khinh khí cầu'.
...
## Task 6: Viết Chương 5 — CCC ↔ Tích hợp + Năng lực chung

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 2,200-2,800 từ.

- [ ] **Step 1: Viết §5.1-5.2 — 7 CCC và gap trong GDPT 2018 (~1,400 từ)**
...
| "So sánh, phân loại, lựa chọn được các sự vật, hiện tượng theo các tiêu chí khác nhau" | CCC 1 Patterns (phần nào) |
| "Phân tích được các đặc điểm của một sự vật, hiện tượng theo logic nhất định" | CCC 4 Systems (phần nào) |
...
- [ ] **Step 2: Viết §5.3 — Bảng đối chiếu chi tiết 7 CCC (~800 từ)**
...
```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 5 — CCC ↔ tích hợp liên môn + năng lực chung

§5.1 7 CCC NGSS, §5.2 khoảng trống CCC trong GDPT 2018 + cách bù,
§5.3 bảng đối chiếu chi tiết 7 CCC với khuyến nghị triển khai.
...
## Task 7: Viết Chương 6 — Phẩm chất chủ yếu và văn hoá STEM

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 1,200-1,500 từ.

- [ ] **Step 1: Viết toàn chương 6**
...
- [ ] **Step 2: Commit chương 6**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 6 — Phẩm chất chủ yếu và văn hoá lớp học STEM

§6.1 5 phẩm chất GDPT 2018 biểu hiện trong KHTN, §6.2 nguyên tắc
văn hoá OpenSciEd ↔ phẩm chất VN, §6.3 hàm ý biên soạn.
...
## Task 8: Viết Chương 7 — Đánh giá

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 1,500-1,800 từ.

- [ ] **Step 1: Viết toàn chương 7**
...
- [ ] **Step 2: Commit chương 7**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 7 — Đánh giá: NGSS 3D vs GDPT 2018

§7.1 Performance Expectation đánh giá 3 chiều NGSS, §7.2 đánh giá
năng lực GDPT 2018 (nhắc Thông tư 22/2021 và 27/2020), §7.3 tương
thích và xung đột khi đánh giá OpenSciedNEO theo chuẩn VN.
...
## Task 9: Viết Chương 8 — Hàm ý triển khai OpenSciedNEO
...
- [ ] **Step 2: Commit chương 8**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 8 — Hàm ý triển khai OpenSciedNEO

§8.1 cho giáo viên: bổ sung phẩm chất, địa phương hoá, YCCĐ, thí
nghiệm; §8.2 cho tổ trưởng/QL: mapping 4 mạch, phê duyệt sổ điểm,
phối hợp liên môn; §8.3 rủi ro hệ thống + khuyến nghị thí điểm.
...
## Task 10: Viết Chương 9 — Phụ lục

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 2,500-3,500 từ. Chia thành 4 sub-task theo A/B/C/D.

- [ ] **Step 1: Viết §9.A — Glossary (~600 từ)**
...
- [ ] **Step 2: Viết §9.B — Trích YCCĐ đầy đủ (~1,500 từ)**

Run trước: `python3 -c "import json; d=json.load(open('docs/source-pdfs/gdpt-2018/yccd-parsed.json')); [print('Lớp', g, ':', len(v['topics']), 'topics') for g,v in d.items()]"`
Mục đích: biết còn bao nhiêu topic để quyết chiến lược trích.

Nếu tổng > 100 topic (dự kiến) → trích **gọn theo cấu trúc mục, không full-text**.
...
- [ ] **Step 3: Viết §9.C — NGSS PE code reference (~500 từ)**
...
- [ ] **Step 4: Viết §9.D — Tham khảo (~300 từ)**
...
- [ ] **Step 5: Commit chương 9**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 9 — Phụ lục A/B/C/D

§9.A Glossary NGSS ↔ GDPT 2018, §9.B trích danh mục chủ đề KHTN
lớp 6-7-8 (không lớp 9 ngoài scope), §9.C NGSS PE code reference
với bảng nhanh code ↔ GDPT 2018, §9.D tham khảo gồm TT 32/2018,
TT 22/27/26 (nhắc tên), NGSS 2013, NRC Framework 2012.
...
## Task 11: Viết Chương 1 — Tóm lược (sau cùng)

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md`

**Budget từ**: 500-800 từ. Viết sau cùng khi đã có toàn bộ chương 2-9, giờ tóm tắt được.

- [ ] **Step 1: Thay placeholder chương 1 bằng nội dung thật**

Find and replace trong file:
...
- [ ] **Step 2: Commit chương 1**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md
git commit -m "Chương 1 — Tóm lược (viết sau cùng)

§1.1 về tài liệu + phạm vi K-8, §1.2 ba kết luận cốt lõi: (a) DCI
khớp tốt THCS lệch mạnh ESS, (b) SEP khớp khung GDPT 2018 nhẹ hơn
ở Models và Argument, (c) CCC là khoảng trống cần bù. §1.3 năm
khuyến nghị hành động triển khai OpenSciedNEO.
...
## Task 12: QA pass — anchor links, cross-refs, word count

**Files:**
- Modify: `docs/NGSS_GDPT2018_BRIDGE.md` (nếu có fix)
- Modify: `docs/NGSS_GDPT2018_BRIDGE_DESIGN.md` (check các checkbox progress)

- [ ] **Step 1: Check word count**

Run: `wc -w docs/NGSS_GDPT2018_BRIDGE.md`
Expected: 18,000-23,000 từ. Nếu < 17,000 → thiếu; nếu > 25,000 → quá dài.

- [ ] **Step 2: Check anchor link TOC**

Test mỗi link trong TOC có target tồn tại:

```bash
grep -oE '\(#[a-z0-9-]+\)' docs/NGSS_GDPT2018_BRIDGE.md | sort -u > /tmp/toc_links.txt
grep -oE '^#+.*$' docs/NGSS_GDPT2018_BRIDGE.md | sed 's/^#\+ *//' | sed 's/[^a-zA-Z0-9 ]//g' | tr '[:upper:]' '[:lower:]' | tr ' ' '-' | sed 's/^/#/' | sort -u > /tmp/toc_targets.txt
diff /tmp/toc_links.txt /tmp/toc_targets.txt
```

Expected: không có dòng chỉ nằm bên trái (link mồ côi). Nếu có, fix anchor trong TOC (hoặc sửa tiêu đề).

- [ ] **Step 3: Check term consistency**

Search các thuật ngữ chính có dùng nhất quán:

```bash
echo "--- YCCĐ variants ---"
grep -oE 'YCCĐ|yêu cầu cần đạt|Yêu cầu cần đạt|Yêu Cầu Cần Đạt' docs/NGSS_GDPT2018_BRIDGE.md | sort | uniq -c
echo "--- GDPT 2018 variants ---"
grep -oE 'GDPT 2018|Chương trình GDPT 2018|CT GDPT 2018' docs/NGSS_GDPT2018_BRIDGE.md | sort | uniq -c
echo "--- KHTN variants ---"
grep -oE 'KHTN|Khoa học tự nhiên' docs/NGSS_GDPT2018_BRIDGE.md | sort | uniq -c
```

Expected: YCCĐ là biến thể chính (viết tắt), đầy đủ dùng "Yêu cầu cần đạt"; GDPT 2018 là biến thể chính; KHTN là biến thể chính. Nếu thấy mixed case lộn xộn → fix.

- [ ] **Step 4: Spot check citation YCCĐ trong file với source**

Chọn 3 YCCĐ bất kỳ đã trích trong §3.4 và §9.B, grep trong source text:

```bash
grep -n "sự nóng chảy, sự đông đặc, sự bay hơi" docs/source-pdfs/gdpt-2018/ctkhtn-6-9.txt
grep -n "tiến hành được thí nghiệm về sự chuyển thể" docs/source-pdfs/gdpt-2018/ctkhtn-6-9.txt
```

Expected: tìm thấy exact string (có thể khác dấu cách/gạch ngang). Nếu không → đã viết sai, fix.

- [ ] **Step 5: Update design spec checkbox**

Edit `docs/NGSS_GDPT2018_BRIDGE_DESIGN.md`, section "Kết cấu kiểm tra", mark all checkbox completed nếu tất cả tasks đã xong.

- [ ] **Step 6: Commit QA fixes (nếu có)**

```bash
git add docs/NGSS_GDPT2018_BRIDGE.md docs/NGSS_GDPT2018_BRIDGE_DESIGN.md
git commit -m "QA pass — anchor links, term consistency, citation spot-check

- Fix X anchor mismatch
- Normalize term variants (YCCĐ, GDPT 2018, KHTN)
- Verify 3 YCCĐ citations match source text
- Mark design spec checkboxes completed

Word count: XX,XXX từ (target: 18,000-23,000)

Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>"
git push origin main
```

Nếu không có thay đổi nào cần fix → skip commit, chỉ log kết quả QA.

---

## Sau khi hoàn thành

- Báo user tổng word count cuối cùng + link GitHub.
- Hỏi user có cần viết thêm chương hoặc refine phần nào không.
- Nếu user OK → cập nhật memory `project_openstemneo_status.md` với deliverable mới này.

---

## Ghi chú cho người thực thi (subagent hoặc executing-plans)

- **Task 1 và Task 12** có phần technical (chạy python, grep), các task còn lại thuần viết content markdown.
- **Commit riêng mỗi task** để user Tuấn theo dõi tiến độ trên GitHub giữa các chương.
- Mỗi step đã có sẵn nội dung markdown cụ thể — chỉ cần copy-paste vào file và thay các placeholder XX nếu có. KHÔNG tự bịa thêm.
- Các ví dụ/tên lesson/số lớp trong file plan này đã được kiểm chéo với Thông tư 32/2018 (source `/tmp/ctkhtn.txt`) và kiến thức chung về NGSS. Nếu phát hiện sai → dừng lại báo user, không tự sửa bừa.
- Disclaimer §3.2.1 về TH **PHẢI giữ nguyên** — quan trọng để tránh hiểu sai.
- Không viết emoji trong file chính (chỉ dùng ✅/⚠️ trong thống kê nếu cần).
